...
def find_square(reader: str, query: str, card_num: int = 1) -> tuple[str, str] | None:
    ...
    result = process.extractOne(q, list(candidates.keys()),
...
def check_new_lines(reader: str, checked_ids: set[str],
                    already_awarded: set[str],
                    card_num: int = 1) -> list[tuple[str, str, list[str]]]:
    """
...
def auto_detect(
    ...
    if reader == "alex":
        ...
        if finished and book.get("format") in ("chapter_book", "early_chapter_book",
                                                "dense_middle_grade", "dense_classic"):
            triggered.append(("W_FINISH_BOOK", f"Finished {book['title']}"))
            if book_total_pages >= 100:
                triggered.append(("W_100_PAGE_BOOK", f"Finished {book['title']} ({book_total_pages} pages)"))
        ...
        if books_finished_this_week >= 3:
            triggered.append(("W_3_BOOKS_WEEK", f"Finished {books_finished_this_week} books this week"))
        ...
        if finished and is_prose and book_first_session_date:
            ...
            try:
                ...
                if (d2 - d1).days <= 2:
                    triggered.append(("M_3_DAYS", f"Finished {book['title']} in {(d2-d1).days+1} days"))
            except Exception:
                pass
    ...
    if card_num == 2:
        if reader == "alex":
            ...
            if finished and book_total_pages >= 150 and book.get("format") in ("chapter_book", "early_chapter_book", "dense_middle_grade", "dense_classic"):
                triggered.append(("W2_150_PAGE_BOOK", f"Finished {book['title']} ({book_total_pages} pages)"))
            if books_finished_this_week >= 3:
                triggered.append(("W2_3_BOOKS_WEEK", f"Finished {books_finished_this_week} books this week"))
            if books_finished_total >= 10:
                triggered.append(("W2_10_BOOKS", f"Finished {books_finished_total} books this summer"))
            if finished and is_prose and book_first_session_date:
                ...
                try:
                    ...
                    if (d2 - d1).days <= 1:
                        triggered.append(("W2_2_DAYS", f"Finished {book['title']} in {(d2-d1).days+1} days"))
                except Exception:
                    pass
            ...
            if finished and book_total_pages >= 400 and book.get("format") in ("chapter_book", "dense_middle_grade", "dense_classic"):
                triggered.append(("M2_400_PAGE_BOOK", f"Finished {book['title']} ({book_total_pages} pages)"))
            if finished and book.get("classification") == "masterpiece":
            ...
            if books_finished_total >= 5:
                triggered.append(("M2_5_BOOKS", f"Finished {books_finished_total} books this summer"))
            if books_finished_total >= 10:
                triggered.append(("M2_10_BOOKS", f"Finished {books_finished_total} books this summer"))
            if finished and is_prose and book_first_session_date:
                from datetime import date as _date
                try:
                    d1 = _date.fromisoformat(book_first_session_date)
                    d2 = _date.fromisoformat(session_date)
                    if (d2 - d1).days <= 1:
                        triggered.append(("M2_2_DAYS", f"Finished {book['title']} in {(d2-d1).days+1} days"))
                except Exception:
                    pass
