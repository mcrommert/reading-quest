...
PLAYERS_CONFIG = [
    {"key": r["key"], "name": r["name"], "monogram": r["monogram"],
...
def find_book(query: str):
    ...
    result = process.extractOne(norm_q, list(idx.keys()),

                                scorer=fuzz.token_set_ratio, score_cutoff=72)
...
def _process_bingo(reader: str, session_pages: int, session_minutes: int,

                   book: dict, finished: bool, book_key: str,

                   session_date: str, audiobook: bool = False) -> list[str]:
    ...
    triggers = bingo_mod.auto_detect(
        ...
        reader, session_pages, session_minutes, daily_pages,

        book, finished, book_first_date, session_date, books_finished_week,
...
def _award_bingo_lines(reader: str, out_lines: list[str], session_date: str = None, card_num: int = 1):
    ...
    for line_id, line_label, square_labels in new_lines:
        ...
        db.add_bingo_points(reader, bonus,

                            f"Bingo {line_label}", session_date=session_date)
        ...
        out_lines.append(
            ...
            f"✨ **Bingo line complete! +{bonus} pts** "
    ...
    if len(awarded_now) >= bingo_mod.total_lines():
        ...
        out_lines.append("")
        out_lines.append(f"🎉 **BINGO CARD #{card_num} COMPLETE!** {reader.capitalize()} finished every line!")
...
def _post_realtime_summaries(reader: str, today: date):
    ...
    gcal.post_daily_summary(

        reader, stats["pts"], stats["pages"], streak, today)
    ...
    reader_pts = [(get_reader(k)["name"], db.get_weekly_stats(k, today=today)["pts"])
...
def _when_label(sess_date: date, today: date) -> str:
    ...
    if 0 < delta < 7: return _DAY_SHORT[sess_date.weekday()]
...
def _recent_sessions_enriched(reader: str, limit: int = 3, book_idx: dict = None,
...
def _avg_ppp(reader: str, since: str = None):
    ...
    q = ("SELECT COALESCE(SUM(points),0) AS pts, "
...
def _build_player(p_cfg: dict, today: date, book_idx: dict, stats_from: str = None) -> dict:
    ...
    squares_checked_idx = [
        ...
        i for i, (sid, _) in enumerate(bingo_squares) if sid in bingo_checked
...
def _build_board_payload() -> dict:
    ...
    return {
        ...
        "milestones": [
            ...
            {"pts": p, "label": l} for p, l in zip(MILESTONE_PTS, MILESTONE_LABELS)
...
async def slash_read(request: Request):
    ...
    if book is None:
        ...
        if fmt_hint in ("chapter_book", "early_chapter_book", "dense_middle_grade",

                        "dense_classic", "nonfiction"):

            db.add_custom_book(key, title, inline_lexile, fmt_hint, "standard",

                               [normalize(title)])

            book = {"key": key, "title": title, "lexile": inline_lexile or 0,

                    "format": fmt_hint, "classification": "standard"}

        else:

            db.add_custom_book(key, title, None, fmt_hint, "standard", [normalize(title)])

            book = {"key": key, "title": title, "lexile": 0,
    ...
    if fmt in scoring.SPECIAL_OVERRIDES:
        ...
        score_line = (
            ...
            f"{pages} pages × {ppp} pts/pg (override) = **{fmt_pts(points)} pts**"
    ...
    day_label = (
        _fmt_today_label(session_date_local) if is_backdated else "Today"
    )
    ...
    bingo_lines = _process_bingo(
        ...
        reader, pages, minutes, book, finished,

        book["key"], session_date_local.isoformat(), audiobook=audiobook
...
async def slash_reading(request: Request):
    ...
    for reader in readers:
        ...
        next_ms = next(
            ...
            ((t, msg) for t, msg in MILESTONE_THRESHOLDS if total["pts"] < t), None)
        ...
        lines.append(
            ...
            f"This week: {fmt_pts(weekly['pts'])} pts · {weekly['pages']} pages"
...
async def slash_book(request: Request):
        ...
        if book is not None:
            ...
            if updated:
                ...
                return _ephemeral(
                    ...
                    f"✅ Updated **{book['title']}** → Lexile {set_lexile}L\n" + ppp
            ...
            db.add_custom_book(key, title, set_lexile, "chapter_book", "standard",

                               [normalize(title)])

            book = {"key": key, "title": title, "lexile": set_lexile,

                    "format": "chapter_book", "classification": "standard"}
    ...
    if book.get("interest_level") == "UG":
        ...
        if _warn:
            ...
            lines.append(f"⚠️ _Upper grades (9+) — parent preview recommended for {' & '.join(_warn)}_")
...
async def slash_bingo(request: Request):
    ...
    if sub in ("check", "uncheck", "show", "audit"):
        ...
        if not reader:
            ...
            return _ephemeral(f"Unknown reader **{parts[1]}**. Use one of: {', '.join(READER_KEYS)}.")
    ...
    if sub == "audit":
        ...
        for sid, label in squares:
            ...
            if s and s.get("checked"):
                ...
                lines.append(f"  ✅ **{label}** — {method} @ {ts}" +
    ...
    if sub == "uncheck":
        ...
        if ok:
            ...
            return _ephemeral(f"↩️ Unchecked **{label}** for {reader.capitalize()}. "
...
async def alexa_skill(request: Request):
    ...
    app_id = (body.get("context") or {}).get("System", {}).get("application", {}).get("applicationId")
    ...
    if req_type == "LaunchRequest":
        return _alexa_response(
            "Welcome to Reading Bot! Say something like: "
            "Alex read 30 pages of Dog Man.",
            end_session=False,
        )
    ...
    if intent_name != "LogReadingIntent":
        return _alexa_response("Sorry, I didn't understand that. Try saying: Alex read 30 pages of Dog Man.")
    ...
    if book is None:
        ...
        book = {"key": key, "title": title, "lexile": 0, "format": "chapter_book", "classification": "standard"}
    ...
    db.log_session(reader, book["key"], book["title"], pages, minutes, ppp, points,
                   finished=finished, session_date=today_local.isoformat())
