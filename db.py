...
def log_session(reader, book_key, book_title, pages, minutes, ppp, points,

                finished=False, session_date: str = None, audiobook=False):
    ...
    with get_conn() as conn:
        ...
        for _col in ("finished INTEGER NOT NULL DEFAULT 0",
                     "audiobook INTEGER NOT NULL DEFAULT 0",
                     "season TEXT NOT NULL DEFAULT '2026-summer'"):
        ...
        existing = conn.execute(
            ...
            (reader, book_key, pages, today, 1 if finished else 0,
             1 if audiobook else 0, cutoff),
        ).fetchone()
        if existing:
        ...
        conn.execute(
            ...
            (reader, book_key, book_title, pages, minutes, ppp, points, today, now,

             1 if finished else 0, 1 if audiobook else 0, CURRENT_SEASON),
...
def bingo_get_state(reader: str) -> list[dict]:
    ...
    with get_conn() as conn:
        ...
        rows = conn.execute(
            ...
            "SELECT * FROM bingo_state WHERE reader=?", (reader,)
...
def update_custom_book_lexile(key: str, lexile: int) -> bool:
    ...
    with get_conn() as conn:
        ...
        cur = conn.execute(
            ...
            "UPDATE books_custom SET lexile=? WHERE key=?", (lexile, key)
